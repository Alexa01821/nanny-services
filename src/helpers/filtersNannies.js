...
const filtersNannies = (nannies, filter) => {
  if (filter === "A to Z") {
    ...
    const filtered = [...nannies].sort((a, b) => a.name.localeCompare(b.name));
    if (filtered.some((item) => item.price_per_hour <= 10)) {
      return filtered.filter((item) => item.price_per_hour <= 10);
    } else {
      Notify.failure('Price "less than 10$"  was not found');
    }
    return filtered;
